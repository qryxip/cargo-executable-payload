...
pub enum Opt {
    #[structopt(
        ...
        author,
        usage(
            r#"cargo b64encode [OPTIONS]
    cargo b64encode [OPTIONS] --src <PATH>
    cargo b64encode [OPTIONS] --bin <NAME>"#,
        )
    )]
    B64encode {
    ...
    match &*bin_targets(metadata).collect::<Vec<_>>() {
        ...
        [bins @ ..] => bail!(
            "could not determine which binary to choose. Use the `--bin` option or `--src` option \
             to specify a binary.\n\
             available binaries: {}\n\
             note: currently `cargo-b64encode` does not support the `default-run` manifest key.",
            bins.iter()
...
fn format_with_template(original_source_code: &str, payload: &str) -> String {
    format!(
        r#"//! This code is generated by [cargo-b64encode](https://github.com/qryxip/cargo-b64encode).
//!
